...
length = []
mydict = {}
mybooks = {"Title":[], "Length":[]}
while title.lower() != 'q':
    title = input("What is the title of the book? (or q to quit): ")
    if title != 'q':
        mybooks["Title"].append(title.title())
        mybooks["Length"].append(len(title))
...
for idx, (Title, Length) in enumerate(mybooks.items()):
    print(f"{idx+1}: '{Title}', has {Length} characters.")
print(50 * '=')
